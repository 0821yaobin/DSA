...
void countingSortByID(Car cars[], int n) {
    int maxID = 0;
    for (int i = 0; i < n; ++i) {
        if (cars[i].rentalID > maxID) {
            maxID = cars[i].rentalID;
        }
    }
...
int metaBinarySearch(Car cars[], int n, int rentalID) {
    int lg = log2(n - 1) + 1;
    int pos = 0;
    ...
    return (cars[pos].rentalID == rentalID) ? pos : -1;
}
...
void homePage()
{
    ...
    while (!exit) {
        ...
        case 2:
            ...
            cout << "\tWelcome to MMU UWU CAR\n";
            cout << "****************************************\n";
            cout << "Counting Sort + Meta Binary Search\n";
            {
                ...
                int index = metaBinarySearch(cars, n, rentalID);
                if (index != -1) {
                    cout << "\nCar found:";
